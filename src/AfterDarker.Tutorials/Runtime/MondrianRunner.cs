...
/// <summary>
/// Shared, inspectable execution path for lessons 08 and 09: prepare and map
/// relocated bytes, supply guest records, set startup registers, initialize,
/// then optionally continue calling MODULE in the SAME persistent guest.
/// </summary>
public static class MondrianRunner
{
    ...
    public static Result Execute(byte[] file, TextWriter? output = null, bool trace = false,
        ...
        Win16Drawing? drawing = null, Action<Func<ushort, PhaseResult>>? afterInitialization = null)
    {
        ...
        var bindings = Win16Imports.BindImports(image, Gateway, enableDrawing: drawing is not null);
        ...
        ushort blankEnd = WriteCaller(BlankCaller, module, DrawingResult, BlankMessage);
        ushort drawEnd = WriteCaller(DrawCaller, module, DrawingResult, DrawFrameMessage);
        ...
        // Initialization does not draw. Lesson 09 supplies a persistent surface
        // for this same HDC before continuing into BLANK and DRAWFRAME.
        ...
        // The callback runs while this guest, heap, registers and surface are
        // alive. Recreating a guest for each frame would reset its animation.
        afterInitialization?.Invoke(message => message switch
        {
            BlankMessage => RunPhase("BLANK", BlankCaller, blankEnd, DrawingResult, HostData),
            DrawFrameMessage => RunPhase("DRAWFRAME", DrawCaller, drawEnd, DrawingResult, HostData),
            _ => throw new NotSupportedException($"Unsupported drawing message {message}.")
        });
